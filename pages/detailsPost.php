...
while ($resultat = mysqli_fetch_array($requete)) {
    ...
    echo '<div class = details-container>'.'<img src=".'.$resultat['img_article'] .'" alt="image de article">' . '<h1>' . $resultat['title'] . '</h1>' . '<div class=articlesContent>' . $resultat['content'] . '</div>' . '</div>' . '<br>';
}
echo "</div>";
 
$requeteComm = mysqli_query($connection, "
    SELECT blog_comments.id_commentaire, blog_comments.content, blog_user.user_pseudo, blog_comments.id_user
    FROM blog_comments
...
while ($resultat = mysqli_fetch_array($requeteComm)) {
  ...
  if ((int)$_SESSION["user"] === (int)$resultat["id_user"] || $_SESSION["role"] === "admin") {
